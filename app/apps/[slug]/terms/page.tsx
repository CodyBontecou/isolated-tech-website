...
async function getApp(slug: string): Promise<App | null> {
  ...
  return queryOne<App>(
    `SELECT id, slug, name, terms_of_service FROM apps WHERE slug = ? AND is_published = 1`,
    [slug],
    env
  );
}
...
export async function generateMetadata({
  ...
  return {
    title: `Terms of Service — ${app.name} — ISOLATED.TECH`,
      ...
      <main className="app-page">
        ...
        <header style={{ marginBottom: "2rem" }}>
          <h1 className="app-page__name">
            Terms of Service<span className="dot">.</span>
          </h1>
          ...
          <article className="legal-content">
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: renderMarkdown(app.terms_of_service),
              }}
            />
          </article>
        ) : (
          <div className="legal-empty">
            <p className="legal-empty__text">No terms of service available.</p>
          </div>
        )}
      </main>
