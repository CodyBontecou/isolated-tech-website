...
import { generateApiKey, isSuperuser } from "@/lib/admin-auth";
...
export async function POST(request: NextRequest) {
  ...
  if (record.status === "complete" && record.user_id) {
    ...
    const user = await queryOne<{
      ...
      `SELECT id, email, name, "isAdmin" as is_admin, COALESCE(is_seller, 0) as is_seller
       FROM user WHERE id = ?`,
      [record.user_id],
      env
    );
    ...
    const canUseCli = user.is_admin === 1 || user.is_seller === 1 || isSuperuser(user.email, env);

    if (!canUseCli) {
      return NextResponse.json(
        { error: "Your account is not enabled for CLI publishing yet" },
        { status: 403 }
      );
    }

    // Generate a user-scoped API key for CLI use
    const { key, expiresAt: keyExpires } = await generateApiKey(env, "cli", user.id);
